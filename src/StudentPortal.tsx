...
function PageLoading({ label = '正在打开' }: { label?: string }) { return <div className="route-loading" role="status"><strong>{label}</strong></div> }
function ScrollAndRouteRecovery() {
  ...
  useEffect(() => {
    const recoverRejectedChunk = (event: PromiseRejectionEvent) => {
      const message = event.reason instanceof Error ? `${event.reason.name}: ${event.reason.message}` : String(event.reason)
      const previousRetryAt = Number(sessionStorage.getItem(CHUNK_RETRY_KEY) || 0)
      if (!CHUNK_ERROR_PATTERN.test(message) || Date.now() - previousRetryAt <= 30_000) return
      event.preventDefault(); sessionStorage.setItem(CHUNK_RETRY_KEY, String(Date.now())); window.location.reload()
    }
    window.addEventListener('unhandledrejection', recoverRejectedChunk)
    return () => window.removeEventListener('unhandledrejection', recoverRejectedChunk)
  }, [])
  return null
}
...
function StudentLearningApp() {
  ...
  return (
    <Layout><ProductionDataCleaner /><div className="route-view route-view--stable"><AppErrorBoundary resetKey={location.key}><Suspense fallback={<PageLoading />}><Routes>
...
export default function StudentPortal() {
  const location = useLocation()
  return <AppErrorBoundary resetKey={location.key}><ScrollAndRouteRecovery /><Suspense fallback={<PageLoading />}><Routes>
