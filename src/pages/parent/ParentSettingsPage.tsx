...
export function ParentSettingsPage() {
  ...
  return <div>
    <PageHeader eyebrow="账号与学生关联" title="绑定与设置" description="必须使用学生端生成的临时绑定码，不能仅凭邮箱查看学生数据。" />
    <div className="content-grid two-equal">
      ...
      <Card><SectionTitle title="权限说明" /><div className="permission-list"><div><Badge tone="success">学生端</Badge><span>看不到家长入口，也不能进入家长路由。</span></div><div><Badge tone="primary">家长端</Badge><span>只能查看已绑定学生的汇总数据。</span></div><div><Badge tone="danger">后端</Badge><span>所有家长接口再次验证 parent 角色和绑定关系。</span></div></div></Card>
    </div>
    <Card><SectionTitle title="已绑定学生" action={<Badge tone="info"><Users size={14} />{children.length} 人</Badge>} />{children.length ? <div className="linked-child-list">{children.map((child) => <div key={child.id}><div className="linked-child-avatar">{child.displayName.slice(0, 1)}</div><div><strong>{child.displayName}</strong><small>{child.email}｜最近同步：{child.lastSyncedAt ? new Date(child.lastSyncedAt).toLocaleString('zh-CN') : '尚未同步'}</small></div><Button variant="danger" size="sm" onClick={() => void unlinkChild(child.id)}><Trash2 size={15} />解除绑定</Button></div>)}</div> : <EmptyState title="暂无已绑定学生" description="让学生在个人设置中生成绑定码，然后在上方输入。" />}</Card>
  </div>
}
