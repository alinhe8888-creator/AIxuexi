...
export function SettingsPage() {
  ...
  const createPairCode = async () => {
    ...
    try {
      ...
      setPairCode(result)
      notify('success', '家长绑定码已生成', '请在 15 分钟内由家长账号使用。')
    } catch (error) {
    ...
    <div>
      ...
      <div className="settings-layout">
        ...
        <div className="stack">
          ...
          <Card>
            <SectionTitle title="绑定家长账号" description="学生只生成临时绑定码，不会看到家长端页面" />
            <Callout title="权限隔离">家长必须使用独立的 parent 账号登录。即使学生直接输入 /parent 地址，前端会跳转，后端家长接口也会返回 403。</Callout>
            {pairCode ? <div className="student-pair-code"><span>家长绑定码</span><strong>{pairCode.code}</strong><small>有效期至 {new Date(pairCode.expiresAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}</small><Button variant="secondary" onClick={() => { void navigator.clipboard.writeText(pairCode.code); notify('success', '绑定码已复制') }}><Copy size={16} />复制绑定码</Button></div> : <Button variant="secondary" onClick={() => void createPairCode()} disabled={pairLoading}><KeyRound size={17} />{pairLoading ? '正在生成…' : '生成 6 位家长绑定码'}</Button>}
          </Card>
