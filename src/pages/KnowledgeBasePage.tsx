...
import { FIXED_SUBJECTS, FIXED_TEXTBOOK_VERSIONS, SUBJECT_DISPLAY_NAMES, type SupportedSubject } from '../config/curriculum'
      ...
      <div className="material-main-grid">
        <Card className="material-upload-card">
          ...
          >
            ...
            <span>{selectedFile ? `${(selectedFile.size / 1024 / 1024).toFixed(1)} MB` : '文件会直接上传到家庭私有 R2，再由后端自动解压和分析'}</span>
          </button>
          <div className="material-meta-grid">
            <label>科目<select value={subjectHint} onChange={(event) => setSubjectHint(event.target.value as typeof subjectHint)}>{subjects.map((item) => <option key={item} value={item}>{item === '自动判断' ? item : SUBJECT_DISPLAY_NAMES[item]}</option>)}</select></label>
            ...
            <label className="wide">教材版本<input disabled value={subjectHint === '自动判断' ? '上传后自动匹配固定版本' : FIXED_TEXTBOOK_VERSIONS[subjectHint]} /></label>
          </div>
          ...
          {serviceStatus && (!serviceStatus.r2Configured || !serviceStatus.models.qwen) && (
            <div className="material-config-warning">
              {!serviceStatus.r2Configured && <span>R2 未配置</span>}
              {!serviceStatus.models.qwen && <span>Qwen 未配置：不能识别教材和建立知识库</span>}
            </div>
          )}
        </Card>
      ...
      {filtered.length ? <div className="material-knowledge-grid">{filtered.map((item) => (
        <Card key={item.id} className="material-knowledge-card" interactive>
          <button onClick={() => setSelected(item)}>
            <div className="badge-row"><Badge tone="primary">{SUBJECT_DISPLAY_NAMES[item.subject as SupportedSubject] || item.subject}</Badge><Badge>{item.grade}</Badge><Badge tone="success">课本资料</Badge></div>
            <h3>{item.title}</h3>
            <p>{item.content}</p>
