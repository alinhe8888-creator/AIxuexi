...
app.get('/api/health', async (_req, res, next) => {
  try {
    ...
    res.json({
      ...
      familyStudentConfigured: Boolean(process.env.FAMILY_STUDENT_EMAIL),
...
app.get('/api/reports/summary', ...studentOnly, asyncRoute(async (req, res) => {
  const snapshot = (await store.getSnapshot(req.user!.id))?.snapshot
  if (!snapshot || typeof snapshot !== 'object' || Array.isArray(snapshot)) {
...
app.use((error: unknown, _req: Request, res: Response, _next: NextFunction) => {
  ...
  res.status(isCors ? 403 : 500).json({
    message: config.nodeEnv === 'production' && !isCors ? '服务器内部错误' : message,
...
async function ensurePrivateAccount(input: {
  ...
  if (await store.findUserByEmail(normalizedEmail)) return
  ...
  console.log(`Created private ${input.role} account: ${normalizedEmail}`)
}

async function start() {
  await ensurePrivateAccount({
    email: (process.env.PRIVATE_STUDENT_EMAIL || '').trim(),
    password: (process.env.PRIVATE_STUDENT_PASSWORD || '').trim(),
    displayName: (process.env.PRIVATE_STUDENT_NAME || '同学').trim(),
    role: 'student',
  })
  await ensurePrivateAccount({
    email: (process.env.PRIVATE_PARENT_EMAIL || '').trim(),
    password: (process.env.PRIVATE_PARENT_PASSWORD || '').trim(),
    displayName: (process.env.PRIVATE_PARENT_NAME || '家长').trim(),
    role: 'parent',
  })

  app.listen(config.port, '0.0.0.0', () => {
    console.log(`AIxuexi API v5.0.0-final listening on port ${config.port}`)
  })
}
