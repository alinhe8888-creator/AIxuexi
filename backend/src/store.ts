...
export const store = {
  ...
  async getSnapshot(studentId: string) {
    ...
    const result = await query<{ snapshot: unknown; updated_at: string }>(
      'SELECT snapshot,updated_at FROM student_snapshots WHERE student_user_id=$1',
      [studentId],
    )
    const row = result.rows[0]
    ...
    if (config.useMemoryDb) {
      ...
      return ids.flatMap((id) => {
        ...
        return [{
          ...
          lastSyncedAt: memory.snapshots.get(id)?.updatedAt ?? null,
        }]
      })
    }
    const result = await query<{
  ...
  async getLinkedStudent(parentId: string, studentId: string) {
    ...
    const result = await query<{
      ...
      `SELECT u.id,u.email,u.display_name,s.snapshot,s.updated_at AS snapshot_updated_at
       ...
       JOIN users u ON u.id=l.student_user_id
       LEFT JOIN student_snapshots s ON s.student_user_id=u.id
       WHERE l.parent_user_id=$1 AND l.student_user_id=$2`,
      [parentId, studentId],
    )
    const row = result.rows[0]
    return row
  ...
  async listRecords(studentId: string, type: string): Promise<StoredRecord[]> {
    if (config.useMemoryDb) {
      return [...(memory.records.get(recordBucketKey(studentId, type))?.values() ?? [])]
    ...
    const result = await query<{
      ...
      `SELECT record_id,record_type,payload,created_at,updated_at
       FROM student_records
       WHERE student_user_id=$1 AND record_type=$2
  ...
  async getRecord(studentId: string, type: string, id: string): Promise<StoredRecord | null> {
    if (config.useMemoryDb) {
      return memory.records.get(recordBucketKey(studentId, type))?.get(id) ?? null
    }
    const result = await query<{
      record_id: string
      record_type: string
      ...
      `SELECT record_id,record_type,payload,created_at,updated_at
       FROM student_records
       WHERE student_user_id=$1 AND record_type=$2 AND record_id=$3`,
      [studentId, type, id],
    )
    const row = result.rows[0]
    return row
  ...
  async upsertRecord(
    ...
    const result = await query<{
      ...
      `INSERT INTO student_records(student_user_id,record_type,record_id,payload,created_at,updated_at)
       ...
       RETURNING record_id,record_type,payload,created_at,updated_at`,
      [studentId, type, id, JSON.stringify(payload)],
    )
    const row = result.rows[0]
  ...
  async deleteRecord(studentId: string, type: string, id: string) {
    if (config.useMemoryDb) {
      memory.records.get(recordBucketKey(studentId, type))?.delete(id)
      return
    }
    await query(
      'DELETE FROM student_records WHERE student_user_id=$1 AND record_type=$2 AND record_id=$3',
      [studentId, type, id],
    )
  },
}
